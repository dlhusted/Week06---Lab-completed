...
public class Week06TicTacToeLab {
  ...
  //initialize scanner in the class section, not main
 static Scanner in = new Scanner(System.in);

  public static void main(String[] args) {
 //declare all of the starting variables   
    String winner = "";
    String player = "X";
    int allSquares = 1;
    int squareNumber;
  ...
  * call the initialize method in the GameBoard class
  * call the displayGameBoard method in the gameboard class  
  */
...
/*
 * while the winner is an empty string and allsquares is less than or equal to 9
 *      (aka) 
 *   declare user input as the value of the input from the scanner
 *   
 * if the number is valid (between 1 and 9)
 *  and if the gameboard replaces the value of the square
 *      and if player is X
 *          switch to player O
 *              or else switch to player X  
 *            allSquares iterates up 1 each turn
 *            display the updated gameboard again
 *  checkWinStatus will return either an empty string "", or "X", or "O", or "draw", depending on the 
 *     values on the gameboard at this current time.          
 *            
 *            
 *       else, if not a valid choice show that the square has already been chosen, and repeat for the same player        
 *   integer.valueOf converts the given user input to a primitive int value   
 *   
 *   
 *   
 *   
 ...
 */
    ...
    while(winner.equals("") && allSquares <= 9) {
      ...
      if(squareNumber > 0 && squareNumber <= 9) {
        if(gameBoard.ifValidSetSquare(squareNumber, input, player)) {
          if(player.equals("X")) {
            player = "O";
          } else {
            player = "X";
 ...
 /*
  * once the compiler has finished all of the above, 
  * print out the following
  * 
  * if winner is "draw", print out that the game is a draw,
  * otherwise print out that the value of winner (x or o) is the winner
  * 
  *    the .equals method checks to see if the characters in one string are the same as a given string
  */
